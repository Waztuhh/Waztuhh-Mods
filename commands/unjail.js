...
module.exports = {
  ...
  async run(client, message, args) {
    try {
      ...
      if (!user) return message.reply({ embeds: [embed.setDescription('Öncelikle jailini kaldırmak istedigin kullanıcıyı belirtmelisin.')] }).catch((err) => console.log(err)).then((e) => setTimeout(() => { e.delete(); }, 10000));
    ...
    if (!user.roles.cache.has(settings.moderasyon.jailrol)) {
      const embedss = new Discord.MessageEmbed()
        .setColor(settings.bot.BotEmbedColor)
        .setFooter({ text: settings.bot.footer})
        .setDescription(`Bu kullanıcı zaten jaili degil?`)
      ...
      const embeds = new Discord.MessageEmbed()
        ...
        .setDescription(`${members}, Kullanıcısının jailini kaldırmak istiyormusunuz?`)
        ...
        collector.on('collect', async b => {
          ...
          if (b.isButton()) {
            if (b.customId === "onay") {
              ...
              message.reply({ embeds: [embed.setDescription(`**${members ? members.toString() : members.username}** - \`(${members.id})\` kullanıcısı ${message.author} tarafından jaili kaldırıldı.`)] }).catch((err) => console.log(err)).then((e) => setTimeout(() => { e.delete(); }, 6000))

              client.channels.cache.get(settings.moderasyon.jaillog).send({
                embeds: [embed.setDescription(`     
    ${members ? members.toString() : members.username} sunucuda jaili kaldırıldı.
    Kullanıcı: ${members ? members.toString() : ""} - \`(${members.id})\`
    Yetkili: ${message.author} - \`(${message.author.id})\`
    Tarih: \`${moment(Date.now()).format("LLL")}\``)]
              });

              members.roles.remove(`${settings.moderasyon.jailrol}`)
               }
            }
            if (b.customId === "red") {
              msg.delete()
